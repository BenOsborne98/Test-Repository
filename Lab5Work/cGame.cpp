...
bool cGame::getInput(bool theLoop)
{
	...
	while (SDL_PollEvent(&event))
	{
		...
		if (event.type == SDL_KEYDOWN)
		{
			...
			case SDLK_d:
			{
				rocketSprite.setRocketVelocity({ 10, 0 });
			}
			break;
			...
			case SDLK_LEFT:
			{
				rocketSprite.setRocketRotation(rocketSprite.getRocketRotation() -10);
			...
			case SDLK_UP:
			{
				FPoint newScale =
				{
				rocketSprite.getSpriteScale().X + 0.1f, rocketSprite.getSpriteScale().Y + 0.1f
				};
			...
			/*case SDLK_UP:
			{
				rocketSprite.setSpriteScale(rocketSprite.getSpriteScale()  );
			}
			break;
